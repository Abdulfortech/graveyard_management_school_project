...
if(isset($_POST['addRequest'])){
  ...
  $query = $conn->query("INSERT INTO requests (user_id, code, name, gender, dob, dod, size, state, graveyard, address, status) 
    VALUE('$user', '$code', '$name', '$gender', '$dob', '$dod', '$size', '$state', '$graveyard', '$address', '$status')") OR die($conn->error);
  
  if ($query) {
    // alert message
    $_SESSION['msg'] = 'The request has been sent';
    echo "<script> window.open('requests', '_self')</script>";
  }else{
    // alert message
